...
/** Đổi `muted`/`loop`/`opacity`/`pointer-events`/`.hidden` của `bgVideoElement` (#bg-video, TÁI
 ...
 * `z-index` khác — dù set opacity:1 đúng, phần tử vẫn KHÔNG render gì cả nếu `.hidden` còn đó. Đây
 * là NGUYÊN NHÂN THẬT của "vẫn không hiện video".
 * SỬA LẦN 2 (21/07/2026, cùng ngày — Giang chỉnh lại: "index đang nằm TRÊN visual effect, phải
 * đưa xuống THẤP HƠN visual") — bỏ HẲN việc set `z-index` qua inline style (bản trước từng nâng
 * lên `15`, TRÊN cả `#visualizer`/`#webgl-canvas` — SUY ĐOÁN SAI trước khi tìm ra nguyên nhân thật
 * là `.hidden`, giờ không còn cần thiết nữa) — trả về ĐÚNG z-index TĨNH mặc định của CSS (`#bg-
 * video { z-index: 0; }`, GIỐNG HỆT cách "Video nền" trang trí vẫn dùng — Giang xác nhận cách đó
 * "vẫn hoạt động tốt") — video nằm DƯỚI `#visualizer` (z:10)/`#webgl-canvas` (z:1), visualizer vẽ
 * ĐÈ LÊN TRÊN video, ĐÚNG ý đồ gốc "visualizer vẫn chạy như thường, TRÊN video đó".
 * `enabled=true`: gỡ `.hidden` (`classList.remove`) + bỏ muted + tắt loop (BẮT BUỘC — cần
 * `bgVideoElement` tự bắn 'ended' để tự chuyển video kế tiếp) + hiện (`opacity:1`) + bật lại
 * `pointer-events:auto` (nhận cử chỉ vuốt — mặc định CSS là `none`, để click xuyên qua trong chế
 * độ nền trang trí).
 * `enabled=false`: thêm lại `.hidden` (an toàn — Block gate đảm bảo Video nền THẬT không hề bật
 * lúc này, xem event/block.js) + trả lại muted+loop=true + ẩn (`opacity:0`) + pointer-events mặc
 * định (`''`, CSS tĩnh `none` tự áp dụng lại).
 * @param {boolean} enabled
 */
function setBgVideoElementForPlayerMode(enabled) {
    bgVideoElement.muted = !enabled;
    bgVideoElement.loop = !enabled;
    bgVideoElement.classList.toggle('hidden', !enabled); // BẮT BUỘC — xem docstring, nguyên nhân thật khiến video không hiện
