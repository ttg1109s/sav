...
const workflowPlaylistFilterPresets = {
    ...
    /** Tính lại `playlistFilterConfig` (giá trị SỐNG, đọc bởi applyPlaylistFilter()) từ 3 field
     * enabled/activePresetId/presets hiện tại — gọi sau MỌI lần đổi 1 trong 3 field đó. KHÔNG tự
     * render/reload gì ở đây (thuần tính state) — nơi gọi tự lo phần đó theo đúng ngữ cảnh. */
    _recomputeLiveConfig() {
        ...
        const preset = enabled ? findPlaylistFilterPresetById(appState.get('playlistFilterPresets'), activeId) : null; // core
        appState.set('playlistFilterConfig', preset ? { ...clonePlaylistFilterConfigDefaults(), ...preset.config } : clonePlaylistFilterConfigDefaults());
        console.log(`writer: "workflowPlaylistFilterPresets._recomputeLiveConfig", page: "playlistFilterConfig", content: "${preset ? `từ preset "${preset.name}"` : 'rỗng (tắt hoặc chưa chọn preset)'}"`);
    },
     ...
     * tắc tổng nếu đang tắt (chọn preset ngầm định nghĩa muốn Filter có hiệu lực), lưu bền rồi hỏi
     * reload (tái dùng workflowPlaylistScope.askReloadToApplyNow() — CÙNG modal Scope). Giữ NGUYÊN
     * màn đang đứng sau khi bấm (KHÔNG tự điều hướng đi đâu, CÙNG khuôn EQ _applyPreset() — có thể
     * chỉnh tiếp rồi Chọn áp dụng lại nhiều lần).
     * @param {string} id */
    async selectPreset(id) {
        ...
        console.log(`writer: "selectPreset", page: "playlistFilterActivePresetId", content: "${id} (\"${preset.name}\"), enabled=true"`);
        this._recomputeLiveConfig();
        await this._persist();
    ...
    async _deletePresetById(id) {
        ...
        if (wasActive) appState.set('playlistFilterActivePresetId', null);
        console.log(`writer: "_deletePresetById", page: "playlistFilterPresets", content: "-${id}${wasActive ? ' (đang active, tự bỏ chọn)' : ''}"`);
        this._recomputeLiveConfig();
        await this._persist();
