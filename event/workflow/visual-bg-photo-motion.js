...
const workflowVisualBgPhotoMotion = {
     ...
     * `visualBgPhotoMotionContainer`/2 layer hiện hành của MÌNH.
    ...
    async _showNext(objectUrl, preset, advanceMs) {
        ...
        this._ensurePointMoveRunner().activateForNewContent(preset, advanceMs); // event/workflow/motion-point-move-runner.js — KHÔNG còn theo layer, activate CHUNG cho cả 2 (trên visualBgPhotoMotionPointMoveWrapper)
        ...
        this._ensureTransitionRunner().runTransition( // event/workflow/motion-transition-runner.js
            visualBgPhotoMotionContainer, outgoingLayer, incomingLayer, preset, advanceMs,
     ...
     * target CỐ ĐỊNH `visualBgPhotoMotionPointMoveWrapper` (VBG luôn sở hữu nó tại chỗ). Toàn bộ điều phối
    ...
    _ensurePointMoveRunner() {
        if (!this._pointMoveRunner) {
            this._pointMoveRunner = createMotionPointMoveRunner(() => visualBgPhotoMotionPointMoveWrapper); // event/workflow/motion-point-move-runner.js, core/dom-refs.js
        }
    ...
    _ensureBeatReactRunner() {
        if (!this._beatReactRunner) {
            this._beatReactRunner = createMotionBeatReactRunner( // event/workflow/motion-beat-react-runner.js
                ...
                () => motionEngineReactLayer, // core/dom-refs.js — GIỮ NGUYÊN tên, dùng chung với Video Player mode
