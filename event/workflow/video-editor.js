...
const workflowVideoEditor = {
    ...
    // ===================== Crop (core/crop-selector.js — DÙNG CHUNG Photo Edit mode) =====================
    // SỬA (31/07/2026, Giang chốt "tái sử dụng, không dùng hai thứ lại cùng tính năng và logic") —
    // bỏ Cropper.js/core/image-editor/cropper-engine.js, dùng core/crop-selector.js — CÙNG cơ chế
    // event/workflow/file-manager-photo.js dùng cho Photo Edit mode (Zoom/Adjust/Draw/Text/Magic).
    // KHÁC Photo ở bước "Áp dụng": Ở ĐÂY KHÔNG cắt pixel nào ngay — chỉ lưu toạ độ dạng tỉ lệ 0-1
    // (`_cropFraction`), video crop THẬT lúc export (mediabunny/WebCodecs, xem processVideo()) —
    // đây là lý do file core dùng chung KHÔNG có hàm "Áp dụng" nào cả, chỉ lo phần tương tác (xem
    // docstring đầu core/crop-selector.js).

    handleCropOpen() {
        if (!appState.get('_nativeW')) return;
        this._pause();
        const w = appState.get('_nativeW'), h = appState.get('_nativeH');
        videoEditorCropSourceCanvasEl.width = w; videoEditorCropSourceCanvasEl.height = h;
        videoEditorCropInteractCanvasEl.width = w; videoEditorCropInteractCanvasEl.height = h;
        videoEditorCropSourceCanvasEl.getContext('2d').drawImage(videoEditorSourceEl, 0, 0, w, h);
        videoEditorCropOverlayEl.classList.remove('hidden');
        this._initCropSelector();
        this._renderCropRatioButtons();
    },

    /** preset tỉ lệ khung hình (Giang yêu cầu: đổi 16:9 <-> 9:16 nhiều lần vẫn phải ra đúng, không
     * biến dạng cộng dồn — xem docstring `setCropSessionAspectRatio()`, core/crop-selector.js). */
    ...
    handleCropRatioSelect(ratio) {
        if (!appState.get('_cropSession')) return;
        setCropSessionAspectRatio(appState.get('_cropSession'), ratio); // core/crop-selector.js
        this._drawVideoCropOverlay();
    },

    /** Khởi tạo session crop (core/crop-selector.js) + khôi phục lựa chọn crop đã lưu trước đó
     * (`_cropFraction`, nếu có — đúng khuôn `ready()` callback cũ của Cropper.js) + gắn Pointer
     * Events lên `videoEditorCropInteractCanvasEl`. */
    _initCropSelector() {
        const w = appState.get('_nativeW'), h = appState.get('_nativeH');
        const session = initCropSession(w, h); // core/crop-selector.js
        const cropFraction = appState.get('_cropFraction');
        if (cropFraction) {
            setCropSessionRect(session, { // core/crop-selector.js
                x: cropFraction.x * w, y: cropFraction.y * h, w: cropFraction.w * w, h: cropFraction.h * h,
            });
        }
        appState.set('_cropSession', session);
        this._drawVideoCropOverlay();

        const getScale = () => videoEditorCropInteractCanvasEl.width / (videoEditorCropInteractCanvasEl.getBoundingClientRect().width || 1);
        const toCanvasPos = (clientX, clientY) => {
            const rect = videoEditorCropInteractCanvasEl.getBoundingClientRect();
            const s = getScale();
            return { x: (clientX - rect.left) * s, y: (clientY - rect.top) * s };
        };
        const handleDown = (e) => cropSessionPointerDown(appState.get('_cropSession'), toCanvasPos(e.clientX, e.clientY), 30 * getScale()); // core/crop-selector.js
        const handleMove = (e) => { this._moveOrResizeCropSession(toCanvasPos(e.clientX, e.clientY)); this._drawVideoCropOverlay(); };
        const handleUp = () => cropSessionPointerUp(appState.get('_cropSession')); // core/crop-selector.js
        videoEditorCropInteractCanvasEl.addEventListener('pointerdown', handleDown);
        videoEditorCropInteractCanvasEl.addEventListener('pointermove', handleMove);
        videoEditorCropInteractCanvasEl.addEventListener('pointerup', handleUp);
        videoEditorCropInteractCanvasEl.addEventListener('pointerleave', handleUp);
        appState.set('_cropPointerCleanup', () => {
            videoEditorCropInteractCanvasEl.removeEventListener('pointerdown', handleDown);
            videoEditorCropInteractCanvasEl.removeEventListener('pointermove', handleMove);
            videoEditorCropInteractCanvasEl.removeEventListener('pointerup', handleUp);
            videoEditorCropInteractCanvasEl.removeEventListener('pointerleave', handleUp);
        });
    },

    /** Đọc `session.activeHandle` rồi CHỌN gọi ĐÚNG 1 trong 3 hàm tính rect thuần của core/crop-
     * selector.js — việc CHỌN này thuộc Workflow (Rule 1 — core không được tự dispatch giữa các
     * tiến trình, xem docstring đầu core/crop-selector.js). ĐÚNG khuôn `workflowFileManagerPhoto::
     * _moveOrResizeCropSession()` (event/workflow/file-manager-photo.js) — file NÀY dùng preset tỉ
     * lệ (16:9/9:16/...) nên nhánh `computeRatioLockedResizedRect()` THẬT SỰ chạy (khác Photo).
     * @param {{x:number,y:number}} pos
     */
    _moveOrResizeCropSession(pos) {
        const session = appState.get('_cropSession');
        if (!session || !session.activeHandle) return;
        const s = session.dragStart;
        const dx = pos.x - s.x, dy = pos.y - s.y;
        const minSize = 50 * (videoEditorCropInteractCanvasEl.width / (videoEditorCropInteractCanvasEl.getBoundingClientRect().width || 1));
    ...
    _drawVideoCropOverlay() {
        const scale = videoEditorCropInteractCanvasEl.width / (videoEditorCropInteractCanvasEl.getBoundingClientRect().width || 1);
        drawCropSessionOverlay(videoEditorCropInteractCanvasEl.getContext('2d'), appState.get('_cropSession'), videoEditorCropInteractCanvasEl.width, videoEditorCropInteractCanvasEl.height, scale); // core/crop-selector.js
    },

    handleCropConfirm() {
        if (!appState.get('_cropSession')) return;
        const rect = getCropSessionRect(appState.get('_cropSession')); // core/crop-selector.js
        const w = appState.get('_nativeW'), h = appState.get('_nativeH');
        appState.set('_cropFraction', { x: rect.x / w, y: rect.y / h, w: rect.w / w, h: rect.h / h });
        appState.set('_hasUnsavedChanges', true);
    ...
    _closeCropOverlay() {
        if (appState.get('_cropPointerCleanup')) { appState.get('_cropPointerCleanup')(); appState.set('_cropPointerCleanup', null); }
        appState.set('_cropSession', null);
