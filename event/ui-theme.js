...
const workflowUiTheme = {
    ...
    async switchUiTheme(themeName) {
        ...
        _mirrorThemeNameToLocalStorage(safeThemeName); // MỚI (Giang yêu cầu "nền preloading theo theme") — xem docstring hàm ngay dưới
    ...
    async loadPersistedUiThemeOnBoot() {
        ...
        const activeThemeName = appConfigUiTheme.getAll().activeUiTheme;
        _mirrorThemeNameToLocalStorage(activeThemeName); // MỚI — xem docstring hàm ngay dưới
...
/**
 * MỚI (Giang yêu cầu tính năng "thiết kế lại preloader, nền theo theme") — ghi lại tên theme đang
 * active vào `localStorage['uiThemeName']` mỗi lần đổi (switchUiTheme())/khôi phục lúc boot
 * (loadPersistedUiThemeOnBoot()) — MIRROR nhanh, ĐỌC ĐỒNG BỘ được, dùng cho script preloader (đầu
 * <body> trong index.html, chạy TRƯỚC MỌI file JS khác, kể cả file này) — preloader KHÔNG THỂ đợi
 * `meta.uiThemeConfig` thật trong IndexedDB (bất đồng bộ, phải vẽ preloader NGAY LẬP TỨC lúc trang
 * vừa mở, trước khi bất kỳ Promise nào kịp resolve). `localStorage` là nguồn DUY NHẤT đọc được kiểu
 * đồng bộ ở thời điểm đó. Nguồn sự thật DUY NHẤT vẫn là `meta.uiThemeConfig` (IndexedDB) — field
 * `localStorage` này CHỈ là bản sao nhanh, KHÔNG đọc lại ở bất kỳ đâu khác trong app ngoài preloader.
 * Bọc try/catch — Safari Private Mode/trình duyệt chặn localStorage vẫn không được làm vỡ luồng
 * đổi/khôi phục theme chính (chức năng CHÍNH không phụ thuộc field phụ này).
 * @param {string} themeName
 */
function _mirrorThemeNameToLocalStorage(themeName) {
    try {
