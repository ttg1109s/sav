 ...
 *
 * MỚI (phản hồi Giang — icon (i) cạnh tiêu đề "Motion", giải thích ngắn gọn các lựa chọn) — dùng
 * `infoIconHtml()` (core/info-icon-ui.js, NẠP SAU file này cũng cần có) — xem
 * 'playerDisplaySettings.motion.infoText' (lang/patch/patch-subtitle-settings.js).
 */
...
function renderPlayerDisplayBody(kind, cfg, motionPresetOptions) {
    ...
    return `
        ...
        <div class="mt-6">
            <h3 class="text-xs font-bold text-sky-600 uppercase tracking-widest mb-2 ml-2 flex items-center gap-1.5">
                <span>${t('playerDisplaySettings.motion.groupTitle')}</span>
                ${infoIconHtml(t('playerDisplaySettings.motion.infoText'))}
            </h3>
            <div class="rounded-2xl flex flex-col overflow-hidden" data-uitk="cardBg cardBorder">
                ${motionRowsHtml}
            </div>
